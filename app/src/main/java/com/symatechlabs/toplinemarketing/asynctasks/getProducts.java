...
public class getProducts extends AsyncTask<String, Integer, String> {
    ...
    public getProducts( AppCompatActivity activity) {
    ...
    protected String doInBackground(String... params) {
        ...
        while (!isCancelled()) {
            ...
            try {
                ...
                HttpPost httpPOst = new HttpPost(ConstantValues.BASE_URL + "get_all_products");
        ...
        if(status){
            try{
                ...
                for (int i = 0 ; i < jsonObject.getJSONArray("products").length() ; i++){
                     productCRUD.add(jsonObject.getJSONArray("products").getJSONObject(i).getString("product_id").trim() , jsonObject.getJSONArray("products").getJSONObject(i).getString("product_name").trim() , jsonObject.getJSONArray("products").getJSONObject(i).getString("company_id").trim());
                }
                if(networkTools.checkConnectivity()){
                    new getSKUs(actionBarActivity).execute();
                }else{
            ...
            Toast.makeText(actionBarActivity , "An error occured retrieving products!!" , Toast.LENGTH_LONG).show();
        }
    }
